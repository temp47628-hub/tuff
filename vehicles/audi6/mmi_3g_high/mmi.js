angular.module('gaugesScreen', [])
  .controller('GaugesScreenController', function($scope, $window, $sce, $timeout) {
    ...
    const PALETTE = {
      BLUE:   { main: '#007eff', alt: fade('#007eff') },
      GREEN:  { main: '#008000', alt: fade('#008000') },
      ORANGE: { main: '#ff1400', alt: fade('#ff1400') },
      YELLOW: { main: '#ffbd00', alt: fade('#ffbd00') },
      RED:    { main: '#ff0000', alt: fade('#ff0000') },
      NONE:   { main: '',        alt: '',             }
    };

	const ASSETS = {
	  mute: 'mute-icon.png',
	  low:  'low-volume-icon.png',
	  high: 'high-volume-icon.png'
	};
	...
	const setTwoLineMessage = (text, color, {
	  slot1 = 2,
	  slot2 = 3,
	  maxChars = 28,
	  left = '105px'
	  ...
	  if (text.length > maxChars) {
		...
		if (words.length > 1) {
		  line1 = '';
		  while (words.length && (line1 + ' ' + words[0]).trim().length <= maxChars) {
			line1 = (line1 + ' ' + words.shift()).trim();
		  }
		  line2 = words.join(' ');
		} else {
		  // For Japanese / Chinese / strings without spaces
		  line1 = text.slice(0, maxChars);
		  line2 = text.slice(maxChars);
		}
	  }

	  $scope.display[`menuItem${slot1}`] = line1;
	  $scope.display[`menuItem${slot2}`] = line2;

	  updateStyle(`.menu-item-${slot1}`, {
		display: 'block',
		color,
		textAlign: 'left',
		left,
	  ...
	  updateStyle(`.menu-item-${slot2}`, {
		display: line2 ? 'block' : 'none',
		color,
		textAlign: 'left',
		left,
		whiteSpace: 'normal',
    ...
    const applyTheme = (theme, activeCorner = null) => {
      ...
      const src = vol === 0 ? ASSETS.mute : vol <= 2.5 ? ASSETS.low : ASSETS.high;
    ...
    const clearInterface = () => {
      ...
      for (let i = 1; i <= 6; i++) {
        updateStyle(`.menu-item-${i}`, { display: 'none', color: '', transform: 'none', left: '', right: '', width: '', textAlign: '', whiteSpace: '', overflow: '' });
    ...
    const SCROLL_ARC = {
      1: { left: 79, bottom: 188, lineWidth: 9 }, 2: { left: 54, bottom: 165, lineWidth: 35 },
      3: { left: 43, bottom: 137, lineWidth: 45 }, 4: { left: 43, bottom: 109, lineWidth: 45 },
      5: { left: 54, bottom:  81, lineWidth: 35 }, 6: { left: 79, bottom:  58, lineWidth: 9 }
    };
	
	const SCROLL_ARC_CLIMATE = {
	  1: { left: 91, bottom: 193, lineWidth: 20 },
	  2: { left: 79, bottom: 165, lineWidth: 20 },
	  3: { left: 73, bottom: 137, lineWidth: 20 },
	  4: { left: 73, bottom: 109, lineWidth: 20 },
	  5: { left: 79, bottom:  81, lineWidth: 20 },
	  6: { left: 91, bottom:  53, lineWidth: 20 }
	};

	const setScrollOverlay = (scroll, color, { startSlot = 1, maxSlots = 6, leftOffset = 0, arc = SCROLL_ARC } = {}) => {
	  const el = find(SEL.scrollOverlay);
	  if (!el) return;
	  el.style.display = 'block';
	  const slots     = maxSlots - startSlot + 1;
	  const slotIndex = ((scroll - 1) % slots) + startSlot;
	  const pos       = arc[slotIndex] || arc[1];
	  ...
	  el.style.setProperty('--scroll-line-width', (pos.lineWidth ?? 20) + 'px');
	};
    ...
    const HANDLERS = {
      [MENU.NAV]: (e) => {
		  const navDisabled = e.navigationDisabled == 1;
		  Object.assign($scope.display, {
			title: navDisabled ? T('Navigation') : '',
			mainLabel: navDisabled ? T('Note') : '',
			menuItem2: navDisabled ? T('Navigation is not installed.') : '',
			cornerBL: navDisabled ? '' : T('Nav-Info'),
			cornerBR: navDisabled ? '' : T('Destination')
		  });
		  updateStyle('body', { backgroundImage: `url('${navDisabled ? 'blue_no_ui.png' : 'hud.png'}')` });
		  if (navDisabled) {
			  setTwoLineMessage(T('Navigation is not installed.'), PALETTE.BLUE.main);
			} else {
			  updateStyle('.menu-item-2', { display: 'none' });
			  updateStyle('.menu-item-3', { display: 'none' });
			}
      ...
      [MENU.CLIMATE]: (e) => {
        ...
        if (isAir) {
          updateStyle('.menu-item-4', { color: (isPass ? airflow2 : airflow1) === 0 ? 'white' : PALETTE.ORANGE.main });
			...
			setScrollOverlay(e.mmiscroll, PALETTE.ORANGE.main, { arc: SCROLL_ARC_CLIMATE });
        }

        updateStyle('body', { backgroundImage: `url('${(e.mmi_detailView || 0) === 11 && !isActive ? 'orange_alt_line_no4_ui.png' : 'orange_alt_noscroll_no4_ui.png'}')` });
      },
      ...
      [MENU.CAR]: (e, d) => {
        ...
        const infoStyle = { display: 'block', color: 'red', left: '105px', right: '0', textAlign: 'left' };
		...
		const pressureState = (raw) => {
            ...
            if (circ) {
              ...
              if (dial) {
                ...
                dial.innerHTML = Array.from({ length: maxVal + 1 }, (_, i) => {
                  ...
                  if (i === 0) {
                    return `<text x="${x}" y="${y}" text-anchor="middle" dominant-baseline="middle"
                      font-size="14" font-weight="bold" font-family="inherit" fill="${color}">${T('dark')}</text>`;
                  } else if (i === maxVal) {
                    return `<text x="${x}" y="${y}" text-anchor="middle" dominant-baseline="middle"
                      font-size="14" font-weight="bold" font-family="inherit" fill="${color}">${T('bright')}</text>`;
                  } else {
                    return `<circle cx="${x}" cy="${y}" r="4" fill="${color}" />`;
                  }
                }).join('');
		  ...
		  updateStyle('body', { backgroundImage: "url('red_line_noscroll_no12_ui.png')" });
			...
			updateStyle('body', { backgroundImage: "url('red_no12_ui.png')" });
          }

          activateSubmenuSelector(3, 'select2.png');
        } else if (activeSub === 4) {
		  Object.assign($scope.display, {
			mainLabel: D('VERSION_INFO', 'mainLabel', T('Version information')),
			menuItem1: D('VERSION_INFO', 'line1', T('Software version:')),
			menuItem2: D('VERSION_INFO', 'line2', 'HNav_EU_K0257_6_D1'),
			menuItem3: D('VERSION_INFO', 'line3', T('Nav. database version:')),
			menuItem4: D('VERSION_INFO', 'line4', '8R0060884GH ECE 6.29.1')
		  });
		  const offsets = [28, 33, 52, 57]; // 1-2: 9px gap, 2-3: 25px gap, 3-4: 9px gap
		  [1, 2, 3, 4].forEach((i, idx) => updateStyle(`.menu-item-${i}`, { ...infoStyle, transform: `translateY(${offsets[idx]}px)` }));
		  updateStyle('body', { backgroundImage: "url('red_line_noscroll_no12_ui.png')" });
      ...
      [MENU.NAME]: (e) => {
        if (e.phoneEnabled != 1) {
          ...
          setTwoLineMessage(T('Telephone is not installed.'), PALETTE.GREEN.main);
          updateStyle('.center-label', { textAlign: 'left', left: '105px' });
      ...
      [MENU.PARKING_GRAPHIC]: (e, d, isCam) => {
        ...
        const parkingBg = isCam ? 'hud2.png' : hasCam ? 'parking_system_cam.png' : 'parking_system.png';
    ...
    $window.updateData = (data) => {
      $scope.$evalAsync(() => {
        ...
        if (timeEl) timeEl.style.transform = ignition < 2 ? 'none' : 'translateX(11px)';
    ...
    (function init() {
      ...
      $window.preloads = [
        'hud.png', 'hud2.png', 'blue_no_ui.png', 'green_no_ui.png',
        'red_line_ui.png', 'red_line_no12_ui.png', 'red_line_noscroll_no12_ui.png',
        'red_no12_ui.png',
        'orange_ui.png', 'orange_line_ui.png', 'orange_alt_no4_ui.png', 'orange_alt_line_no4_ui.png', 'orange_alt_noscroll_no4_ui.png',
        'green_no2_ui.png', 'green_line_no2_ui.png',
		...
		...['mute-icon.png', 'low-volume-icon.png', 'high-volume-icon.png']
