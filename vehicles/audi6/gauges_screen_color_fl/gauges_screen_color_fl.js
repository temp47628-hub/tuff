...
.controller('GaugesScreenController', function($scope, $window, $sce) {
  ...
  // Logic state variables for context overrides
  ...
  $window.updateData = (data) => {
    ...
    $scope.$evalAsync(function () {
      ...
      if (!e.manualShifterEnabled) {
        if (e.gear === -1) { $scope.data.gear = "R"; } 
        else if (e.gear === 0) { $scope.data.gear = "N"; } 
        else { $scope.data.gear = e.gear; }
      }
      ...
      let songName = e.audi6_currentSongName || "";
      $scope.data.toptext = songName.length > 18 ? songName.substring(0, 15) + "..." : songName;
